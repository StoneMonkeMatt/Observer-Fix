...
function validateStructuralSummary(summary: Record<string, unknown>, errors: string[]): void {
  ...
  if ("highDualityPersistence" in summary && typeof summary.highDualityPersistence !== "boolean") {
    errors.push("structuralSummary.highDualityPersistence must be a boolean.");
  }
}
...
function validateEventLog(eventLog: unknown[], errors: string[]): void {
  const validTypes = new Set<EventType>([
    ...
    "sequence_stabilized",
    "bridge_event",
    "high_duality_candidate",
    "high_duality_persistence",
    "structural_warning",
    "collapse_warning",
    "adversarial_failure",
  ]);
